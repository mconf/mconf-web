...
class PostTest < ActiveSupport::TestCase
  ...
  def test_should_parse_atom
    ...
    params = Post.atom_parser(data)
    assert params.include?(:entry)
    assert params.include?(:post)
    assert params.include?(:tags)
    assert_equal "title", params[:post][:title]
    assert_equal "contenido", params[:post][:text]
  ...
  protected
    ...
    <id>tag:localhost,2005:Post/11</id>  
    <published>2008-10-02T12:33:50+02:00</published>  
    <updated>2008-10-02T12:33:50+02:00</updated>  
    <link type=\"text/html\" rel=\"alternate\" href=\"/spaces/2/posts/11\"/>  
    <link type=\"application/atom+xml\" rel=\"self\" href=\"/spaces/2/posts/11.atom\"/>  
    <title>#{ title }</title>  

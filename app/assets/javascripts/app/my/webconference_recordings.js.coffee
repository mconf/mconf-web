$ ->
  if isOnPage 'my', 'webconference_recordings'
...
submitFormUpdate = ->
  ...
  $form.on "ajax:success", (evt, data, status, xhr) ->
    $('#my-webconference-recordings-wrapper').html(data)
...
showErrorNotification = (xhr) ->
  if xhr?.responseText? and !_.isEmpty(xhr.responseText.trim())
    ...
    if response?.message?
      ...
      msg = I18n.t("my.webconference_recordings.update_recordings.error") + ': ' + response.message
      console.log msg

...
describe InvoicePostWorker, type: :worker do
  ...
  describe "#perform" do
    before { worker.should_receive(:invoices_post)
             worker.should_receive(:invoices_sync) }
    it { worker.perform }
  end
    ...
    context "invoice.flag_invoice_status == Invoice::INVOICE_STATUS[:local]" do
      ...
      context "calls post invoice" do
        before {
          ...
          Invoice.any_instance.should_receive(:post_invoice_to_ops).once
        }
        it "invoice.post_invoice_to_ops" do
          worker.invoices_post
        end
      end

      context "calls post invoice without receiving data" do
        before {

...
describe UserMailer do
  ...
  describe '.new_user_waiting_for_approval' do
    ...
    let(:mail) { UserMailer.waiting_for_approval_email(user.id) }

    context "in the standard case" do
      it("sets 'to'") { mail.to.should eql([user.email]) }
      it("sets 'subject'") {
        text = I18n.t("user_mailer.waiting_for_approval_email.subject")
    ...
    context "uses the receiver's locale" do
      ...
      it {
        content = I18n.t("user_mailer.waiting_for_approval_email.subject", locale: "pt-br")
  ...
  context "calls the error handler on exceptions" do
    ...
    it {
      with_resque do
        ...
        Mconf::MailerErrorHandler.should_receive(:handle).with(UserMailer, nil, exception, "waiting_for_approval_email", anything)
        UserMailer.waiting_for_approval_email(1).deliver
      end
    }
    it {
      with_resque do

class MartchRecordingsToMeetingsCloserCreateTime < ActiveRecord::Migration
  ...
  def find_matching_meeting_closer_create_time(recording)
    ...
    if meeting_id.nil?
      meeting = BigbluebuttonMeeting.where("meetingid = ? AND created_at > ? AND created_at < ?", recording.meetingid, Time.at(recording.start_time)-2.minutes, Time.at(recording.start_time)+2.minutes).last
      if meeting.nil?
        meeting_id = nil
      else

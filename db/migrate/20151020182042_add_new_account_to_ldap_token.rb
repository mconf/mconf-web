class AddNewAccountToLdapToken < ActiveRecord::Migration
  def up
    add_column :ldap_tokens, :new_account, :boolean, default: false
    ...
    # base code. In this case we assume the user was created via ldap.
    puts "Automatically setting tokens as :new_account if they match the default criteria"
    LdapToken.joins(:user)
      .where("TIMESTAMPDIFF(SECOND, users.created_at, ldap_tokens.created_at) < 10")
      .find_each do |token|
  ...
  def down
    remove_column :ldap_tokens, :new_account
  end
end

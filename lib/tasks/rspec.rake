...
spec_prereq = File.exist?(File.join(::Rails.root.to_s, 'config', 'database.yml')) ? "setup:basic_data:test" : :noop
task :noop do
end
...
namespace :spec do
  ...
  namespace :db do
    namespace :fixtures do
      ...
      task :load => :environment do
        ...
        (ENV['FIXTURES'] ? ENV['FIXTURES'].split(/,/) : Dir.glob(File.join(::Rails.root.to_s, 'spec', 'fixtures', '*.{yml,csv}'))).each do |fixture_file|
  ...
  namespace :server do
    daemonized_server_pid = File.expand_path("spec_server.pid", ::Rails.root.to_s + "/tmp")
